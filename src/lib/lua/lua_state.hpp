...
class LuaState {
	public:
		...
		explicit LuaState(lua_State* state) : L(state) {}
...
class LuaField {
	...
	public:
		...
		) : lua(L), index(idx), stackIndex(stackIdx) {}
		LuaField(LuaState &L, const std::string idx) : lua(L), index(idx), stackIndex(-2) {}
...
class LuaClass {
	...
	public:
		LuaClass(
			LuaState &L,
			std::string name
		) : lua(L), className(std::move(name)) {}

		LuaClass(
			LuaState &L,
			std::string  name,
			std::string  base
		) : lua(L), className(std::move(name)), baseClass(std::move(base)) {}

		LuaClass(
			LuaState &L,
			std::string  name,
			const lua_CFunction constructorFunction
		) : lua(L), className(std::move(name)), constructor(constructorFunction) {}
...
class LuaMethod {
	...
	public:
		...
		) : lua(L), className(std::move(name)), methodName(std::move(method)), func(func), metaMethod(meta) {}
