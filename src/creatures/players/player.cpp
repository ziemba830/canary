	...
	if (zone == ZONE_PROTECTION) {
		...
		if (getLevel() > g_configManager().getNumber(ADVENTURERSBLESSING_LEVEL)) {
			for (uint8_t i = 2; i <= 8; i++) {
				if (!hasBlessing(i)) {
					sendCreatureSay(static_self_cast<Player>(), TALKTYPE_MONSTER_SAY, "WARNING: Blessings are not active.", &getPosition());
					sendCreatureSay(static_self_cast<Player>(), TALKTYPE_MONSTER_SAY, "Type !bless to buy blessings.", &getPosition());
					break;
				}
			}
		}
