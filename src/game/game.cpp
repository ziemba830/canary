...
bool Game::internalCreatureSay(Creature* creature, SpeakClasses type, const std::string &text, bool ghostMode, SpectatorVector* spectatorsPtr /* = nullptr*/, const Position* pos /* = nullptr*/) const {
	...
	if (!spectatorsPtr || spectatorsPtr->empty()) {
		...
		if (type != TALKTYPE_YELL && type != TALKTYPE_MONSTER_YELL) {
			...
			if (pos->z < 8) {
				map.getSpectatorsInternal(spectators, *pos, (Map::maxClientViewportX + 1) * 2, (Map::maxClientViewportX + 1) * 2, (Map::maxClientViewportY + 1) * 2, (Map::maxClientViewportY + 1) * 2, 0, 7, false);
			} else {
				map.getSpectatorsInternal(spectators, *pos, (Map::maxClientViewportX + 1) * 2, (Map::maxClientViewportX + 1) * 2, (Map::maxClientViewportY + 1) * 2, (Map::maxClientViewportY + 1) * 2, pos->z, pos->z, false);
			}
		}
	} else {
...
void Game::notifySpectators(const SpectatorVector &spectators, const Position &targetPos, Player* attackerPlayer, Monster* targetMonster) {
	if (!spectators.empty()) {
		for (Creature* spectator : spectators) {
			if (!spectator) {
				continue;
...
void Game::sendEffects(
	...
	const SpectatorVector &spectators
) {
	uint8_t hitEffect;
